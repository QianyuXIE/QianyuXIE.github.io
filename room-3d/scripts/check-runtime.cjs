...
(async () => {
  ...
  const dependencies={
    './room-note.js':{createWallNote:()=>null},
  ...
  assert(floor.receiveShadow,'Open ground receives real shadows');
  assert(!renderer.scene.getObjectByName('studio_backdrop'),'No rear wall or curved sweep');
  assert(!renderer.scene.getObjectByName('glass_shadow_receiver'),'No finite vertical shadow rectangle');
  assert.equal(model.getObjectByName('studio_glass_wall').visible,false);
  assert(renderer.scene.fog.far < 100,'Ground fades before the camera far plane');
  ...
  assert(shader.vertexShader.includes('studioWorldPosition = (modelMatrix'),'Scenic shading is anchored in world coordinates');
  assert(shader.fragmentShader.includes('diffuseColor.rgb *= 1.0 - shade'),'Scenic shade retains standard lighting and fog');
